...
function requireWallet(
  wallet: ReturnType<typeof useWallet>["publicKey"]
): string {
...
>(config: {
  ...
  const apiFn = client.governance[config.method] as (...args: any[]) => any;
...
export function useClosePositionMutation() {
  return useGovernanceMutation({
    ...
    buildTag: (p: any) =>
      `gov-close-${hashTagParams({ position: p.positionMint })}`,
  });
}
